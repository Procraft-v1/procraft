...
| `https://dashboard.procraft.uz/` | Static **`apps/web`** SPA + `try_files` fallback to `index.html` |
| `https://admin.procraft.uz/` | Static **`apps/admin`** SPA + `try_files` fallback to `index.html` |
| `https://username.procraft.uz/` | Static **`apps/profiles`** bundle (subdomain vhost map) |
| `https://api.procraft.uz/api/*` | `apps/api` container (`proxy_pass`) |
| `https://api.procraft.uz/uploads/*` | ASP.NET static file middleware or object storage gateway |

Implementation notes:

- Enable **sticky nothing** requirement for SPA; caching headers differ (`landing` long-cache assets, dashboard/admin no-store HTML if needed during releases).
...
Uploaded files stay in the API container volume, but the browser opens them through `https://api.procraft.uz/uploads/...`; nginx forwards that path to the ASP.NET static file middleware.
